...
def build_strategy_name(config):
    """
    Build a complete strategy name from configuration including stand name.
    
    Examples:
        "sparse_sparse_stand_4-row_start5"
        "dense_stand_3-row_start2_Thin from Below_0.25"
        "kentucky_stand_5-row_start3_Thin by CI1 (Distance-Dependent Competition)_0.15"
    """
    # Extract stand name from file path
    stand_file = config['input']['stand_file']
    # Get filename without path and extension
    stand_filename = os.path.basename(stand_file)
    # Remove .csv and _with_feet suffixes if present
    stand_name = stand_filename.replace('.csv', '').replace('_with_feet', '')
    # Clean up the name - remove extra .csv if present
    if '.csv' in stand_name:
        stand_name = stand_name.split('.csv')[0]
    
    primary_config = config['primary_thinning']
    primary_strategy = primary_config['strategy']
    start_row = primary_config['start_row']
    
    # Start with stand name + primary strategy
    strategy_name = f"{stand_name}_{primary_strategy}_start{start_row}"
...
def calculate_thinning_statistics(df_thinned, columns):
    ...
    # QMD (Quadratic Mean Diameter) - FIXED
    # QMD = sqrt(mean of squared diameters)
    pre_qmd = float(np.sqrt((alive[dbh_col]**2).mean())) if len(alive) > 0 else np.nan
    post_qmd = float(np.sqrt((kept[dbh_col]**2).mean())) if len(kept) > 0 else np.nan
...
def plot_mortality_map(df_original, df_after_thinning, growth_df, output_path, final_dbh_col, columns):
    """
    Create spatial visualization showing mortality after growth projection
    Gray = thinned trees (context), Green = alive after growth, Red = dead during growth
    
    Args:
        df_original: Original stand before thinning (no thin_decision column)
        df_after_thinning: Stand after both primary+secondary thinning (df_final, has thin_decision)
        growth_df: Growth projections (same length as df_after_thinning)
        final_dbh_col: Column name for final DBH
        columns: Column mapping
    """
    xcol = columns['x_coord']
    ycol = columns['y_coord']
    
    # 1. Get thinned trees by set difference
    original_alive = df_original[df_original['status'] == 'Alive'].copy()
    kept_trees = df_after_thinning[df_after_thinning['thin_decision'] == 'Keep'].copy()
    
    # Thinned trees = original alive - kept trees (by index or coordinates)
    kept_indices = set(kept_trees.index)
    thinned_trees = original_alive[~original_alive.index.isin(kept_indices)].copy()
    
    # 2. Add mortality status from growth_df to kept_trees
    kept_trees = kept_trees.reset_index(drop=True)
    growth_status = growth_df[final_dbh_col].reset_index(drop=True)
    
    # Separate alive vs dead during growth
    alive_mask = growth_status.notna()
    alive_trees = kept_trees[alive_mask].copy()
    dead_trees = kept_trees[~alive_mask].copy()
    
    # Create figure
    fig, ax = plt.subplots(figsize=(10, 8))
    
    # Plot layers: background to foreground
    
    # 1. Thinned trees (gray, background)
    if len(thinned_trees) > 0:
        ax.scatter(thinned_trees[xcol], thinned_trees[ycol], 
                  s=6, label='Thinned (removed)', alpha=0.25, 
                  color='lightgray', edgecolors='gray', linewidths=0.2)
    
    # 2. Trees alive after growth (green)
    if len(alive_trees) > 0:
        ax.scatter(alive_trees[xcol], alive_trees[ycol], 
                  s=12, label='Alive', alpha=0.8, color='green')
    
    # 3. Trees dead during growth (red, foreground)
    if len(dead_trees) > 0:
        ax.scatter(dead_trees[xcol], dead_trees[ycol], 
                  s=18, label='Dead (Mortality)', alpha=0.95, 
                  marker='x', color='red', linewidths=1.8)
    
    # Format plot
    ax.set_aspect('equal', adjustable='box')
    ax.set_xlabel('X (feet)', fontsize=10)
    ax.set_ylabel('Y (feet)', fontsize=10)
    ax.set_title('Final Stand: Mortality Visualization', fontsize=12, fontweight='bold')
    
    # Calculate stats (from post-thinning population)
    total_after_thinning = len(kept_trees)
    n_alive = len(alive_trees)
    n_dead = len(dead_trees)
    survival_rate = (n_alive / total_after_thinning * 100) if total_after_thinning > 0 else 0
    mortality_rate = (n_dead / total_after_thinning * 100) if total_after_thinning > 0 else 0
    
    ax.legend(
        title=f'Survival: {survival_rate:.1f}% | Mortality: {mortality_rate:.1f}%',
        loc='best', 
        fontsize=9,
        framealpha=0.9
    )
...
def run_complete_workflow(config_file):
    ...
    # Add coordinates to growth_df for mortality map
    # growth_df and df_final must have matching indices
    xcol = columns['x_coord']
    ycol = columns['y_coord']
    ...
    if config['output'].get('create_maps', True):
        no_years_temp = stand_params['projection_age'] - stand_params['current_age']
        final_dbh_col_temp = f'DBH+{no_years_temp}'
        mortality_map = os.path.join(run_dir, "06_final_stand_mortality_map.png")
        
        # Pass original stand (for thinned context), final stand (after thinning), and growth data
        plot_mortality_map(df_stand, df_final, growth_df, mortality_map, final_dbh_col_temp, columns)
        
        if verbose:
            alive_count = growth_df[final_dbh_col_temp].notna().sum()
            dead_count = growth_df[final_dbh_col_temp].isna().sum()
            print(f"\n✅ Mortality map created:")
            print(f"   Alive: {alive_count:,} trees")
            print(f"   Dead: {dead_count:,} trees")
            print(f"   File: {os.path.basename(mortality_map)}")
    ...
    # Calculate all final metrics (surviving trees only) - MOVED HERE
    survived_mask = growth_df[final_dbh_col].notna()
    final_dbh_values = growth_df.loc[survived_mask, final_dbh_col]
    final_ht_values = growth_df.loc[survived_mask, final_ht_col]
    
    # Mean values for Excel export and summary
    ...
    stand_area_acres = stand_params.get('stand_area_acres', None)
    per_acre_metrics = {}
    ...
    if stand_area_acres is not None and stand_area_acres > 0:
        ...
        per_acre_metrics = {
            'trees_per_acre': float(len(final_dbh_values) / stand_area_acres),
            'volume_per_acre': float(total_volume_final / stand_area_acres),
            'volume_growth_per_acre': float(growth_in_volume / stand_area_acres),
            'tpa_by_dbh_class': {
                'bins': dbh_bins[:-1],  # Lower edges of bins
                'counts': tpa_distribution
            }
        }
    ...
    summary = {
        ...
        'growth_projection': {
            ...
            'final_mean_dbh': float(growth_df[final_dbh_col].mean()),
            'final_mean_height': float(growth_df[final_ht_col].mean()),
            ...
            'final_dbh_sum': sum_final_dbh,
            'final_height_sum': sum_final_height,
            'total_volume_final': total_volume_final,
            'volume_after_thinning': volume_after_thinning,
            'growth_in_volume': growth_in_volume,
            'per_acre_metrics': per_acre_metrics,  # Empty dict if no stand area
        },
        'output_files': {
            ...
            'mortality_map': '06_final_stand_mortality_map.png' if config['output'].get('create_maps') else None,
    ...
    """
    Volume calculations moved up before summary dictionary creation (line ~1538)
    """
    # Build strategy name
    strategy_name = build_strategy_name(config)
    
    # Verbose output for volume (using already-calculated variables)
    if verbose:
        trees_after_thin = len(growth_df)
        trees_survived = survived_mask.sum()
        mortality_count = trees_after_thin - trees_survived
        mortality_pct = (mortality_count / trees_after_thin) * 100
        
        print(f"\n📊 Volume Calculations:")
        print(f"   Trees After Thinning: {trees_after_thin}")
        print(f"   Trees Survived: {trees_survived} ({100-mortality_pct:.1f}%)")
        print(f"   Mortality: {mortality_count} trees ({mortality_pct:.1f}%)")
        print(f"   Volume After Thinning (surviving trees only): {volume_after_thinning:.2f} cubic feet")
        print(f"   Volume at Final Age: {total_volume_final:.2f} cubic feet")
        print(f"   Growth in Volume: {growth_in_volume:.2f} cubic feet")
    ...
    if verbose:
        ...
        print(f"  Mean DBH (Final): {summary['growth_projection']['final_mean_dbh']:.2f} inches")
        print(f"  Mean Height (Final): {summary['growth_projection']['final_mean_height']:.2f} feet")
        print(f"  DBH Sum (Final): {summary['growth_projection']['final_dbh_sum']:.2f} inches")
        print(f"  Height Sum (Final): {summary['growth_projection']['final_height_sum']:.2f} feet")
